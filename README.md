  ...
  <div class="container">
    ...
    <div id="update-note-container" class="panel">
      ...
      <ul class="update-note">
        <li>Start with $400 to make buying stocks harder.</li>
        <li>Stock prices change more (up or down 15%).</li>
        <li>Pay a small fee (1%) to buy or sell.</li>
        <li>Predictions are right only half the time.</li>
        <li>Prices update every 45 seconds.</li>
      </ul>
    </div>
  ...
  <script>
    ...
    function updateStocks() {
      stocks.forEach(stock => {
        ...
        let changePercent;
        if (Math.random() < 0.05) { // 5% chance for market shock
          changePercent = (Math.random() - 0.5) * 30; // ±30% for shock
        } else if (predictedChanges[stock.symbol] && Math.random() < 0.5) {
    ...
    function updateTotalCost() {
      if (selectedStock) {
        ...
        if (actionType === 'buy') {
          quantity = Math.min(quantity, Math.floor(cash / (selectedStock.price * 1.01)));
        ...
        if (document.getElementById('confirmation').style.display === 'block') {
          ...
          const fee = (selectedStock.price * quantity * 0.01).toFixed(2);
    ...
    function showConfirmation(type) {
      ...
      if (type === 'buy' && (cash <= 0 || totalCost + fee > cash)) {
        alert('Not enough money to buy this amount (including 1% fee)!');
        return;
      }
    ...
    document.getElementById('plus-btn').onclick = () => {
      ...
      let quantity = parseInt(quantityInput.value) || 1;
      if (actionType === 'buy') {
        quantity = Math.min(quantity + 1, Math.floor(cash / (selectedStock.price * 1.01)));
      } else if (actionType === 'sell' && portfolio[selectedStock.symbol].shares > 0) {
    ...
    updateCash();
    updatePortfolio();
    setInterval(updateStocks, 45000);
